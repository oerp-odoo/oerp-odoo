...
class MrpUnbuildMulti(models.TransientModel):
    ...
    include_from_group = fields.Boolean("Include on the Same Group")
    ...
    def _find_mos(self):
        ...
        if self.include_from_group:
            for mo in self.mrp_production_ids:
                mos |= mo._get_children()
                mos |= mo._get_sources()
        mos_skipped = (
