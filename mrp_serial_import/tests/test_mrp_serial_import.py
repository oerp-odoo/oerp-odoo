...
class TestMrpSerialImport(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.product_lodge = cls.env.ref('mrp_serial_import.product_product_lodge')
    ...
    def _map_lots_by_name(self, production):
        ...
        return dict(
            productions.mapped(lambda r: (r.lot_producing_id.name, r.lot_producing_id))
        )
    ...
    def test_01_assign_serial(self):
        ...
        wizard = self.StockAssignSerial.with_context(**action['context']).create(
            {'next_serial_number': 'Placeholder'}
        )
        ...
        self.assertEqual(str(lots_map['SN002']['note']), '<p>Special Serial</p>')
