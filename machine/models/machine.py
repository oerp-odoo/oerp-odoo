...
class MachineInstance(models.Model):
    ...
    domain = fields.Char()
    os_instance_ids = fields.One2many(
        'machine.os.instance', 'machine_instance_id')
...
class MachineCpuBrand(models.Model):
    ...
    name = fields.Char(required=True,)
    cpu_vendor_id = fields.Many2one('machine.cpu.vendor', "CPU Vendor")
...
class MachineCpu(models.Model):
    ...
    name = fields.Char(required=True,)
    cpu_brand_id = fields.Many2one('machine.cpu.brand', "CPU Brand")
...
class MachineDbInstance(models.Model):
    ...
    os_instance_id = fields.Many2one(
        'machine.os.instance', "OS Instance", required=True,)
...
class MachineOsInstance(models.Model):
    """Model representing OS instance on machine instance."""

    _name = 'machine.os.instance'
    _description = 'Machine OS Instance'

    name = fields.Char()
    machine_instance_id = fields.Many2one(
        'machine.instance', "Machine Instance", required=True,)
    db_instance_ids = fields.One2many(
        'machine.db.instance', 'os_instance_id', "Database System Instances")
    amount_users = fields.Integer()
