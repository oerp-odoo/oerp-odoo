...
class ProductTemplateHsCode(models.Model):
    ...
    name = fields.Char(compute='_compute_name', string="Country HS Code", store=True)
    ...
    def _compute_name(self):
        for hs in self:
            hs.name = '%s%s' % (hs.product_tmpl_id.hs_code or '', hs.code or '')
        ...
        for hs in self.filtered('code'):
            if not re.match(HS_COUNTRY_CODE_RE, hs.code):
                raise ValidationError(
                    _("Country HS Code Part must be digits only. Got" " '%s' instead")
                    % hs.code
                )
    ...
    def _check_is_origin_country(self):
        for hs in self.filtered('is_origin_country'):
            if len(hs.product_tmpl_id.hs_code_ids.filtered('is_origin_country')) > 1:
                raise ValidationError(_("Only one HS Code can have Origin Country!"))
